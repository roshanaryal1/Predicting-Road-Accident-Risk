...
@st.cache_resource(show_spinner=False)
def get_percentiles():
    """Quantiles of predicted risk across the training set, for placing a
    single prediction in context. Generated by compute_concentration.py."""
    try:
        import numpy as _np
        return _np.load("model/risk_percentiles.npy")
    except Exception:
        return None


def risk_percentile(pred: float):
    """Percentile of this prediction against the training distribution."""
    q = get_percentiles()
    if q is None:
        return None
    import numpy as _np
    return float(_np.searchsorted(q, pred) / len(q) * 100)
...
if page == "🔮 Predict Risk":
    ...
    st.markdown(
        "<p style='color:#8a93a6;font-size:0.88rem;line-height:1.5;margin:-6px 0 14px'>"
        "Supports <b>UN SDG 3.6</b> (halve road deaths and injuries) and <b>11.2</b> "
        "(safe, sustainable transport). <b>Intended for</b> road-controlling authorities "
        "and safety engineers deciding where to direct limited infrastructure budget — "
        "signage, lighting, curve treatment, speed-limit review — rather than spreading "
        "it evenly across a network. Decision-support only; not for individual driver "
    ...
    demo_defaults = dict(DEMO_SCENARIOS[st.session_state.demo_idx])
    _demo_label = demo_defaults.pop("_label", None)
    if _demo_label:
        st.caption(
            f"Demo {st.session_state.demo_idx + 1} of {len(DEMO_SCENARIOS)} — "
            f"{_demo_label}. Click again to cycle."
        )
    ...
    with col_card:
        ...
        _pct = risk_percentile(pred)
        if _pct is not None:
            if _pct >= 90:
                _msg = (f"This condition sits in the **top {100-_pct:.0f}%** of predicted risk. "
                        "The highest-risk 10% of conditions carry **1.82x** their proportional "
                        "share of total predicted risk — this is the tier targeted investment "
                        "would prioritise.")
            elif _pct >= 75:
                _msg = (f"Higher risk than **{_pct:.0f}%** of conditions in the training set, "
                        "but outside the top decile that concentrates 1.82x its share of risk.")
            else:
                _msg = (f"Lower risk than **{100-_pct:.0f}%** of conditions in the training set. "
                        "Not a prioritisation candidate on these inputs.")
            st.info(_msg)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_a:
        raw_a = input_widgets("cmp_a", defaults={k: v for k, v in DEMO_SCENARIOS[0].items() if k != "_label"})

    with tab_b:
        raw_b = input_widgets("cmp_b", defaults={k: v for k, v in DEMO_SCENARIOS[1].items() if k != "_label"})
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    rc1.metric("Top 1%", "2.23×", help="Top 1% of segments carry 2.2% of total predicted risk — 2.23x their proportional share")
    rc2.metric("Top 5%", "1.98×", help="Top 5% of segments carry 9.9% of total predicted risk")
    rc3.metric("Top 10%", "1.82×", help="Top 10% of segments carry 18.2% of total predicted risk")
    rc4.metric("Top 25%", "1.57×", help="Top 25% of segments carry 39.3% of total predicted risk")
    st.caption(
...
highest-risk conditions carry 1.82×** their proportional share of total
predicted risk. Reproducible by running `compute_concentration.py`; full
method and an explicit statement of what this does *not* claim are in
`ssd2026/CONCENTRATION.md` and `ssd2026/IMPACT.md`.
